/*Acknowledgements: https://github.com/rishitSaiya*/
%{
	#include <stdio.h> 
	#include <string.h> 
	#include <stdlib.h> 
				...
				if(variable_declaration==1){
					for(int i = 0; i < symbol_table[scope].num_strs; i++ ){
						if(!strcmp(symbol_table[scope].strs[i].ID,$1)){
							printf("DECLARATION ERROR %s  (previous declaration was at line %d)\n", $1,symbol_table[scope].strs[i].l_no);
							return 0;
						}
					}
					for(int i = 0; i < symbol_table[scope].num_vars; i++ ){
						if(!strcmp(symbol_table[scope].vars[i].ID,$1)){
							printf("DECLARATION ERROR %s  (previous declaration was at line %d)\n", $1,symbol_table[scope].vars[i].l_no);
							return 0;
						}
					}
					$<var_entry>$ = malloc(sizeof(struct VarEntry())); $<var_entry>$->ID = $1; $<var_entry>$->Type = variable_type;$<var_entry>$->l_no = yylineno;
...
id_tail:	',' id  
			{
				if(variable_declaration==1){
					...
					$<var_entry>$ = malloc(sizeof(struct VarEntry())); $<var_entry>$->ID = $2; $<var_entry>$->Type = variable_type;$<var_entry>$->l_no = yylineno;
					symbol_table[scope].vars[symbol_table[scope].num_vars] = *($<var_entry>$); symbol_table[scope].num_vars++;
				}
			}
			id_tail {} | 
